 ...
 */
function risolt(params, time = 500) {
	const results = [];
	const funs = params.map((w) => walkPromise(w, time));
	const _funs = funs.map((f) => f());

	/**
	 * Run the functions in series, meaning that each function will be run after
	 * the previous function has resolved or rejected. If any of the functions
	 * reject, the promise returned by this function will reject with the same
	 * error.
	 *
